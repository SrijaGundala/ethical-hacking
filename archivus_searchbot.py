"""
This script initializes a Flask web application for handling file
uploads, converting PDFs to images,
...
1. Imports:
   ...
   - render_template, request, send_file: Flask functions for rendering HTML
    templates and handling HTTP requests.
...
def view_page_link(category, filename, pagenumber):
    ...
    if os.path.exists(image_path):
        return (f'<a href="/view_image/{category}/{filename}/{pagenumber}"'
                f' target="_blank">View Page</a>')
...
def search_data(db_name, keywords, category):
    ...
    Returns:
        pandas.DataFrame: A DataFrame containing search results with columns:
         'S.NO', 'filename', 'category',
    ...
    if category and category != 'All':
        ...
        query = (
            "SELECT filename, 'Contracts' AS category, pagenumber "
            "FROM contracts WHERE text LIKE ? OR filename LIKE ? "
            "UNION SELECT filename, 'Policies' AS category, pagenumber FROM "
            "policies WHERE text LIKE ? OR filename LIKE ? "
            "UNION SELECT filename, 'GDMS' AS category, pagenumber FROM GDMS "
            "WHERE text LIKE ? OR filename LIKE ? "
            "UNION SELECT filename, 'ISO' AS category, pagenumber FROM iso WHERE"
            " text LIKE ? OR filename LIKE ?"
        )
    ...
    df['View Page'] = df.apply(lambda row: view_page_link
    (row['category'], row['filename'], row['pagenumber']), axis=1)
    df['Download'] = (df.apply
            (lambda row: download_file(BASE_DIRECTORY,
                                       row['category'], row['filename']), axis=1))
    return df


def get_df2(db_name):
    """
        Retrieves a DataFrame containing information about files with
        page number 1 from various categories.

        Args:
            db_name (str): The name of the SQLite database.

        Returns:
            pandas.DataFrame: A DataFrame with columns: 'S.NO',
    ...
    query = ("SELECT filename, 'Contracts' AS category, pagenumber "
             ...
             "'Policies' AS category, pagenumber FROM policies "
             "WHERE pagenumber = 1 UNION SELECT filename, "
             "'ISO' AS category, pagenumber FROM iso "
             "WHERE pagenumber = 1 UNION SELECT filename, "
...
def check_file_exists(db_name, filename):
    ...
    for table in tables:
        cursor.execute(f"SELECT filename FROM {table} WHERE filename=?",
                       (filename,))
...
def pdf_to_images(pdf_path, output_folder):
    """
    Converts each page of a PDF document into individual images and saves
...
def convert_pdfs_to_images(source_folder, DEST_FOLDER):
    """
    Converts PDF files in the source folder to individual images and saves them
...
def search():
    ...
    Retrieves data from the "Archivus.db" database based on the provided keywords
    ...
    return render_template('search_results.html',
                               grouped_html_tables=grouped_html_tables)
        ...
        if file:
            ...
            for page_num, text, source in texts:
                ...
                cursor.execute(f"INSERT INTO {category}"
                               f" (filename, category, pagenumber, text) VALUES (?, ?, ?, ?)",
                               (filename, category, page_num, text_with_source))
            conn.commit()
            conn.close()
            return (f"Sucessfully Inserted text from {filename} in category "
                    f"{category} into database.")
