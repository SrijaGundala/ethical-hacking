...
Modules:
    ...
    base64: Provides functions for encoding binary data to printable ASCII
    characters and decoding them back.
    io: Provides a convenient way to handle in-memory file streams.
    functools: Allows partial application of functions, fixing certain arguments
     and generating a new function.
    pandas: A powerful data manipulation and analysis library for Python. Here
    it's used to read and manipulate Excel data.
    plotly.express: A high-level interface to Plotly, a graphing library. It's
    used for creating interactive plots and visualizations.
    streamlit: A framework for creating web apps in pure Python. Used for displaying
    and interacting with data and visualizations in a web interface.
    difflib: Provides tools for comparing sequences, especially useful for comparing text files.
"""
...
def process_data(files):
    ...
    #         return 'Others'
    #     elif (id[0] in ['5', '6', '9'] and len(id) >= 7) or
    #     (id[:3] == 'N00' and len(id) == 6) or len(id) >= 7:
    ...
    filtered_df['Updated at'] = pd.to_datetime(filtered_df['Updated at'],
                                               format='%H:%M:%S', errors='coerce')
    filtered_df['Verified at'] = pd.to_datetime(filtered_df['Verified at'],
                                                format='%H:%M:%S', errors='coerce')
    filtered_df['HOG Approval at'] = pd.to_datetime(filtered_df['HOG Approval at'],
                                                format='%H:%M:%S', errors='coerce')
    ...
    filtered_df['Updated on'] = pd.to_datetime(filtered_df['Updated on'],
                                               errors='coerce')
    filtered_df['Verified on'] = pd.to_datetime(filtered_df['Verified on'],
                                                errors='coerce')
    filtered_df['HOG Approval on'] = pd.to_datetime(filtered_df['HOG Approval on'],
                                                    errors='coerce')
    filtered_df['HOD Apr/Rej on'] = pd.to_datetime(filtered_df['HOD Apr/Rej on'],
                                                   errors='coerce')
    filtered_df['Clearing date'] = pd.to_datetime(filtered_df['Clearing date'],
                                                  errors='coerce')
    ...
    df['Document No'] = (df['Document No'].apply
                         (lambda x: str(x) if isinstance(x, str) else ''))
    df['Document No'] = (df['Document No'].apply
                         (lambda x: re.sub(r'\..*', '', x)))
    df['HOG Approval by'] = df['HOG Approval by'].astype(str)
    df['HOG Approval by'] = (df['HOG Approval by'].apply
                             (lambda x: str(x) if isinstance(x, str) else ''))
    df['HOG Approval by'] = (df['HOG Approval by'].apply
                             (lambda x: re.sub(r'\..*', '', x)))
    df.reset_index(drop=True, inplace=True)
...
def find_similar_invoice_numbers(filtered_df, threshold=0.7):
    ...
    """
    Finds rows in a DataFrame with invoice numbers in a
    specified column that are at least
    a certain percentage similar to each other.

    Args:
        filtered_df (pandas.DataFrame): The DataFrame containing
        invoice numbers.
        threshold (float, optional): The minimum similarity threshold (0.0 to 1.0).
         Defaults to 0.7.
    ...
    def compare_invoices(inv1, inv2):
        """
        Compares two invoice numbers and returns True if their
         similarity ratio is above the threshold.

        Args:
            inv1 (str): The first invoice number to compare.
            inv2 (str): The second invoice number to compare.

        Returns:
            bool: True if the similarity ratio between the two invoice numbers is
            above the threshold, False otherwise.
        """
        ratio = SequenceMatcher(None, inv1, inv2).ratio()
...
def format_amount(amount):
    ...
    Returns:
    - str: A formatted string representing the amount in Indian Rupees (₹).
           The formatting depends on the length of the integer part of the amount:
           - If the integer length is greater than 5 and less than or equal to 7,
           the amount is divided by
             100,000 and formatted as "₹ {amount / 100000:,.2f} lks".
           - If the integer length is greater than 7, the amount is divided by
           10,000,000 and formatted as
             "₹ {amount / 10000000:,.2f} crs".
           - Otherwise, the amount is formatted directly as "₹ {amount:,.2f}".
    """
...
def line_plot_used_amount(data, category, years, width=400, height=300):
    ...
    def format_amounts(amount_value):
        """
    ...
    for year, amount in zip(amount_length["year"], amount_length["amount_length"]):
        formatted_amount = format_amounts(amount)
        fig.add_annotation(
            x=year,
            y=amount,
...
def check_and_filter_authorization(filtered_df, checked_columns_auth):
    """
    Check the selected columns for authorization and filter the DataFrame accordingly.

    Args:
    - filtered_df (pd.DataFrame): The DataFrame to be filtered.
    - checked_columns_auth (list): The list of columns selected for authorization check.

    Returns:
    - tuple: A tuple containing the filtered DataFrame, the checked columns, and the filename.
    """
...
def is_similar(s1, s2):
    """
    Determine if two strings are similar by removing special characters and
    ...
    Returns:
        bool: True if the cleaned strings are equal, False otherwise.
    """
